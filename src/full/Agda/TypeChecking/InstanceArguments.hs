...
findInScope' m cands = ifM (isFrozen m) (return (Just cands)) $ do
    ...
    case cands of
      ...
      [(term, t')] -> do
        ...
        ca <- liftTCM $ runExceptT $ checkArguments ExpandLast DontExpandInstanceArguments (getRange mv) [] t' t
        case ca of
    ...
    checkCandidateForMeta m t term t' =
      ...
      liftTCM $ flip catchError handle $ do
        ...
        localTCState $ do
          ...
          ca <- runExceptT $ checkArguments ExpandLast DontExpandInstanceArguments  noRange [] t' t
          case ca of
            Left _ -> return False
            Right (args, t'') -> do
